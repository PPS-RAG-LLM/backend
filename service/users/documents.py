...
def _extract_text_and_meta(file_bytes: bytes, filename: str, content_type: str) -> tuple[str, Dict[str, Any]]:
...
async def upload_document(
    *,
    ...
    doc_info_path.write_text(json.dumps(doc_info_json, ensure_ascii=False, indent=2), encoding="utf-8")

    # 4) 청크 분할 + 임베딩 + vector-cache 저장
    chunks = _chunk_text(page_content, max_tokens=512, overlap=64)
    if not chunks:
    ...
    vec_cache_path.write_text(json.dumps(vec_items, ensure_ascii=False, separators=(",", ":")), encoding="utf-8")
