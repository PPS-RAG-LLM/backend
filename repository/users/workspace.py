...
from storage.db_models import Workspace, WorkspaceUser, LlmModel, SystemPromptTemplate, User
...
def get_default_llm_model(category: str) -> Optional[Dict[str, str]]:
    ...
    with get_session() as session:
        stmt = select(LlmModel.provider, LlmModel.name).where(
            LlmModel.category == category,
            LlmModel.is_default == True,
            LlmModel.is_active == True
        ).order_by(LlmModel.id.desc()).limit(1)
    ...
    *,
    ...
    with get_session() as session:
        try:
            ...
            logger.info(f"Workspace inserted: id={workspace.id}, slug={slug}, category={category}")
            return workspace.id
...
def delete_workspace_by_slug_for_user(user_id: int, slug: str) -> bool:
    ...
    with get_session() as session:
        ...
        workspace_id_subquery = select(WorkspaceUser.workspace_id).where(
            WorkspaceUser.user_id == user_id
        ).join(Workspace).where(Workspace.slug == slug)
        
        # 워크스페이스 삭제
        stmt = delete(Workspace).where(
            Workspace.id.in_(workspace_id_subquery)
        )
...
def get_workspace_id_by_slug_for_user(user_id: int, slug: str) -> Optional[int]:
    ...
    with get_session() as session:
        stmt = select(Workspace.id).join(WorkspaceUser).where(
            Workspace.slug == slug,
            WorkspaceUser.user_id == user_id
        ).limit(1)
